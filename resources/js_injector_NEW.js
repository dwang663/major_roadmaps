jQuery(document).ready(function () {
    ...
    jQuery(".tooltip").on("mouseenter", function () {
        ...
        // Horizontal shift
        const elementLeft = jQuery(this).offset().left;
        const elementWidth = jQuery(this).outerWidth();
        const elementCenter = elementLeft + (elementWidth / 2);
        const windowWidth = jQuery(window).width();

        const naturalLeft = elementCenter - (tooltipWidth / 2);
        const naturalRight = naturalLeft + tooltipWidth;

        let shift = 0;
        if (naturalLeft < padding) {
            shift = padding - naturalLeft;
        } else if (naturalRight > windowWidth - padding) {
            shift = (windowWidth - padding) - naturalRight;
        }
        
        tooltip.css("margin-left", (-tooltipWidth / 2) + shift + "px");
        tooltip[0].style.setProperty("--arrow-shift", shift + "px");
    });
